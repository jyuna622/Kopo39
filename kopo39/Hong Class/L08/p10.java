...
public class p10 {
	...
	public static void main(String[] args) throws IOException {
		...
		File k39_f1 = new File ("C:\\Users\\kopo39\\Desktop\\StockDailyPrice.csv");//StockDailyPrice.csv파일로 콤마분리자 파일을 만든다.
		...
		while((readtxt = k39_br.readLine()) != null){//한 줄씩 읽어서 readtxt변수에 저장
			...
			String[] k39_field = readtxt.split("%_%");//배열에 있는 내용을 읽어주고 구분자를 %_% 로 잡아 field배열에 저장
			
			//보이는 구분자(^%_%^)가 아닌듯하다 구분자를 (%_%)로 잡고 각 필드에서 (^)를 날리는 작업으로 처리하였고, 빈칸등 유효하지 않은 레코드가 있어서 단축코드는 A로 반드시 시작하기에 A를 잡아냈다.
			if (k39_field.length > 2 && k39_field[2].replace("^", "").trim().substring(0,1).equals("A")) {
				k39_s.append(k39_field[0].replace("^", "").trim());//append: 추가. --> ^를 빈칸으로 바꿔주고, trim()함수를 활용하여 문자열의 앞과 뒤의 공백을 없애주는 함수. 원본은 손상되지 않음
				for (int j = 1; j < k39_field.length; j++) { //j의 범위는 1부터 끝까지(끝의 크기를 알기위해 field.length를 써줌)
					k39_s.append("," + k39_field[j].replace("^", "").trim());//append: 추가. --> ^를 빈칸으로 바꿔주고, trim()함수를 활용하여 문자열의 앞과 뒤의 공백을 없애주는 함수. 
				}
				k39_bw1.write(k39_s.toString()); //BufferedWriter로 클래스의 toString() 메소드 호출 (즉, String 객체로 변환) --> 그리고 그 내용을 적기
				k39_bw1.newLine();//BufferedWriter로 새로운 줄 생성
		...
		System.out.printf("Programme End [%d][%d] records\n", k39_cnt, k39_wcnt); //프로그램긑났다는 것을 프린트해주고 데이터와 유효한 데이터 프린트
